...
## Release Center 第一版（#050~#057）

Release Center 不进入 RuleChain，也不替代 EdgeTask 执行通道。第一版新增平台侧 `ReleasePlan`、`ReleaseTask` 和 `ReleaseReceipt`：

- `POST /api/ReleaseCenter/Plans`：创建发布计划，支持 `SoftwareUpdate`、`DeviceScriptOta`、`FirmwareOta`、批次大小、确认策略、自动开始和 Gateway/EdgeNode/Device/AssetScope/DeviceScope 目标。
...
- Device 表示单个设备 OTA 执行目标
- DeviceScope 保留为设备范围型任务枚举值，Release Center 会先展开为单 Device EdgeTask 后下发
