...
| `edge-task-v1` | `EdgeTaskRequestDto`、`EdgeTaskDto`、`EdgeTaskReceiptDto`、`EdgeTaskStateMachine` | `IoTSharp.Contracts/edge-task.v1.schema.json` | `IoTSharp.Contracts/examples/edge-task.v1.sample.json`、`edge-task.software-update.v1.sample.json`、`edge-task.device-script-ota.v1.sample.json`、`edge-task.firmware-ota.v1.sample.json`、`edge-task.firmware-ota.receipt.v1.sample.json`、`edge-task.firmware-ota.rollback-receipt.v1.sample.json` |
...
- 固件 OTA 成功回执还必须证明设备侧 bootloader 已验收：`firmwarePartition` 或 `activePartition` 对齐任务分区，`bootloaderVersion` 回传实际版本，`bootloaderAccepted=true`。非回滚固件任务必须回传 `rollbackReady=true`，回滚固件任务必须回传 `rollbackConfirmed=true`。任务参数中的范围式 `bootloaderVersion` 由执行端解释，平台只对精确版本做相等校验。
