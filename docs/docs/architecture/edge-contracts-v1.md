...
| `release-package-v1` | `ReleasePackageDto`、`ReleasePackagePublishRequestDto`、`ReleasePackagePublishResultDto` | `IoTSharp.Contracts/release-package.v1.schema.json` | `IoTSharp.Contracts/examples/release-package.v1.sample.json` |
...
- `release-package-v1` 目前只覆盖 #046 最小软件包元数据；灰度、批次、审批、回滚策略留给 M5 Release Center 扩展。
...
- ReleasePackage 合同描述软件包元数据、版本、目标运行时和校验和；具体发布动作仍通过 `SoftwareUpdate` EdgeTask 承载，M5 的灰度和审批不进入实时规则链。
