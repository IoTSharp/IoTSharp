...
export class SlidecontrolComponent implements OnInit {
  ...
  constructor(private el: ElementRef, private controlService: ControlService, @Inject(ALAIN_I18N_TOKEN) private i18n: I18NService) {}
  ...
  touchEnd(e: any) {
    ...
    this.controlService.vertifyAuthImage(this.controlInput.firstConfirmUrl, query).subscribe(
      (data: Result) => {
        this.result = { ...data };
  ...
  draw() {
    this.controlService.getAuthImage(this.controlInput.genUrl).subscribe(
      (data: Result) => {
        this.result = { ...data };
        if (this.result.code === 10000) {
