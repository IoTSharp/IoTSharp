...
public sealed class SonnetMqEventBusWorker : BackgroundService
{
    ...
    {
        var messages = _store.Pull(subscription.Topic, _options.ConsumerGroup, Math.Max(1, _options.PullBatchSize));
        if (messages.Count == 0)
            return false;
        ...
        foreach (var message in messages)
        {
            ...
            try
            {
                ...
                _store.Ack(subscription.Topic, _options.ConsumerGroup, message.Offset);
            }
            catch (Exception ex)
            {
                _logger.LogError(
    ...
    private static Task DispatchAsync(ISubscriber subscriber, string kind, SonnetMqMessage message)
        => kind switch
        {
    ...
    private static Task DispatchConnectAsync(ISubscriber subscriber, SonnetMqMessage message)
    {
        var status = ReadJson<DeviceConnectStatus>(message);
        return subscriber.Connect(status.DeviceId, status.ConnectStatus);
    }

    private static Task DispatchActiveAsync(ISubscriber subscriber, SonnetMqMessage message)
    {
        var status = ReadJson<DeviceActivityStatus>(message);
        return subscriber.Active(status.DeviceId, status.Activity);
    }

    private static T ReadJson<T>(SonnetMqMessage message)
    {
