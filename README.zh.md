...
### 🖥️ 5 分钟 SonnetDB Profile 体验

首次本地试用时，默认从 SonnetDB Profile 开始。该路径只需要 IoTSharp 与 SonnetDB 两个容器，关系库、遥测库、缓存、对象存储和平台事件队列都指向 SonnetDB：
...
该 Profile 使用 `ASPNETCORE_ENVIRONMENT=SonnetDB` 并挂载 `appsettings.SonnetDB.json`。传统 PostgreSQL / TimescaleDB Compose 仍保留用于兼容性和生产拓扑验证，但快速体验默认优先走 SonnetDB。

### 🖥️ 本地运行 IoTSharp

建议先从以下文档入口开始：

- 快速开始：<https://iotsharp.net/guide/getting-started/>
- 部署指南：<https://iotsharp.net/guide/deployment/>
...
如果你明确需要 SQLite 体验实例，也可以让 OpenClaw 引导你完成本地 SQLite 版 IoTSharp 的安装。建议直接给它下面这两样内容：
