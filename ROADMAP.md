...
M1（#010~#018）、M2（#020~#029）和 M3（#030~#038）已完成；下一步进入 M4 #040 Collection Configuration Version。并行轨道 T（#070~#072）为文档层面工作，可随时穿插。

M3 已关闭，采集模板和边缘执行链路已经形成最小闭环。M4 可以开始配置版本与发布任务模型；M5 仍需等待 M4 的配置发布、回执和最小软件包能力稳定后再启动正式实现。
