...
| #038 | `🚧` | 边缘执行链路对齐 | 边缘 | 平台已可从 Product Collection Template 生成 `collection-config-v1` 并携带 `ProductCollectionTemplate` 来源信息；IoTEdge 仓仍需按该载荷完成执行验收。 |
...
M1（#010~#018）已完成，M2 的 EdgeNode 正式模型、API/前端主路径、平台侧跨仓契约固化、IoTEdge 任务回执转正和 #029 semantic-core 收敛决策已完成；下一步进入 M3 #030 CollectionTemplate。并行轨道 T（#070~#072）为文档层面工作，可随时穿插。

M2 已关闭，M3 平台侧 #030~#037 已落地，#038 进入 IoTEdge 执行验收。M4 和 M5 仍需等待 M3 的采集模板和边缘执行链路稳定后再启动正式实现。
