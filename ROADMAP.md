...
| P | `🚧` | CoAP 接入 route 化（并行轨道） | MQTT 接入保持现状；CoAP 独立补 route 风格入口、低分配 payload 处理和平台侧业务分发。 |
...
| #09A | `⬜` | CoAP 性能与压测 | 平台 | 覆盖 CoAP path 匹配 benchmark、JSON source generation、payload `ReadOnlyMemory<byte>`、小对象池化策略、Blockwise 大 payload 和 DTLS/Observe smoke。 |
| #09B | `⬜` | 文档收口 | 平台 | 输出 CoAP 接入说明、新旧路径差异表和运维配置说明；明确旧短路径不兼容、MQTT 不属于本轨道改造范围。 |
...
M1（#010~#018）、M2（#020~#029）、M3（#030~#038）、M4（#040~#048）和 M5（#050~#057）已完成。M4 已具备采集配置版本快照、模板发布到 EdgeTask、执行端目标配置获取、执行回执、配置版本差异展示、失败重试、审计、平台侧最小软件包发布、IoTEdge 软件更新执行器和边缘数据断网缓存/续传。M5 Release Center 平台第一版已形成 EdgeNode/Gateway 运行时软件发布、设备脚本 OTA 和设备固件 OTA 平台闭环；并行轨道 T（#070~#072）已完成单依赖体验、基准报告和云边同底座参考部署。

M3、M4 和 M5 已关闭，采集模板、边缘执行链路、最小软件包发布、边缘数据可靠性和 Release Center 第一版已经形成最小闭环。M5 在 #046 最小模型和 #047/#048 执行端能力上扩展了发布计划、灰度、回滚、暂停继续、确认策略和审计，不进入 RuleChain。#056 设备脚本 OTA 与 #057 设备固件 OTA 的平台合同、范围发布、Gateway 投递通道和回执核对已落在 IoTSharp。

并行轨道 P（CoAP 接入 route 化）、E（附属运行时硬化）与 A（离线 AI 底座）按带宽穿插推进，不阻塞主线；其中 P 轨道只处理 CoAP 接入命名、route 风格入口和性能优化，MQTT 接入保持现状；#090（AI Provider 抽象）是 M6 #064/#065 与 #093 内网部署的前置。

#072 已在本仓库沉淀参考部署与平台侧验收文档；P 轨道以 #096~#09B 验收 CoAP route 风格入口与性能优化；其他涉及 IoTEdge（#028、#038、#047、#048、#080~#084、#095）、IoTEmbedded（#056、#057、#085~#087）和 Tomur（#091、#092、#094）的事项在各自仓库实现，验收标准和契约版本以本路线图与 `IoTSharp.Contracts` 为准。
