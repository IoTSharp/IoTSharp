...
| RD-10 | 🚧 | SonnetDB 可选数据底座生产化 | ✅ 已完成 RD-10.1：`TelemetryStorage=SonnetDB` 覆盖写入、最新值、历史查询、聚合、健康检查、备份恢复和故障回放；⬜ 兼容矩阵、服务端聚合/latest 下推、批量写入、长稳和回滚仍未完成；暂不把每设备 measurement 改为共享 measurement + `deviceId` TAG 作为默认路线。 |
