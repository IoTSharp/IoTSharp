...
可与任何里程碑并行。目标是把「全栈零外部依赖、云边同底座」确立为对外首要卖点，同时保证 SonnetDB 负责数据库引擎、查询执行、备份恢复和维护能力，IoTSharp 只负责平台数据映射、稳定接口适配、权限、审计与受控运维编排。
...
M1（#010~#018）、M2（#020~#029）、M3（#030~#038）、M4（#040~#048）和 M5（#050~#057）已完成。M4 已具备采集配置版本快照、模板发布到 EdgeTask、执行端目标配置获取、执行回执、配置版本差异展示、失败重试、审计、平台侧最小软件包发布、IoTEdge 软件更新执行器和边缘数据断网缓存/续传。M5 Release Center 平台第一版已形成 EdgeNode/Gateway 运行时软件发布、设备脚本 OTA 和设备固件 OTA 平台闭环；并行轨道 T 的 #070~#072 已完成单依赖体验、基准报告和云边同底座参考部署，#073~#076 正在整改 SonnetDB 集成边界。
