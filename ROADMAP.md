...
| M5 | `🚧` | Release Center 第一版 | EdgeNode/Gateway 运行时软件发布、灰度批次、回滚、确认、回执和审计已具备平台第一版闭环；设备脚本/固件 OTA 留在 #056/#057。 |
...
M5 排在 Edge 和采集闭环之后。它不进入 RuleChain。#050 在 #046 最小模型上兼容扩展，不重建。第一版先闭环 EdgeNode/Gateway 运行时软件发布；Device、Asset 范围、设备脚本和固件 OTA 作为后续执行端能力继续推进。
...
| #052 | `✅️` | ReleaseTask | 平台 | 已新增发布任务模型并绑定 EdgeTask；第一版支持 EdgeNode/Gateway 运行时目标，Device、AssetScope、DeviceScope 保留为设备 OTA 和范围发布后续。 |
...
M1（#010~#018）、M2（#020~#029）、M3（#030~#038）和 M4（#040~#048）已完成。M4 已具备采集配置版本快照、模板发布到 EdgeTask、执行端目标配置获取、执行回执、配置版本差异展示、失败重试、审计、平台侧最小软件包发布、IoTEdge 软件更新执行器和边缘数据断网缓存/续传。M5 Release Center 平台第一版（#050~#055）已进入实现并形成 EdgeNode/Gateway 运行时软件发布闭环；并行轨道 T（#070~#072）为文档层面工作，可随时穿插。

M3 和 M4 已关闭，采集模板、边缘执行链路、最小软件包发布和边缘数据可靠性已经形成最小闭环。M5 在 #046 最小模型和 #047/#048 执行端能力上继续扩展发布计划、灰度、回滚、暂停继续、确认策略和审计，不进入 RuleChain。下一步重点转向 #056/#057 的设备脚本/固件 OTA 执行端契约与范围发布能力。
