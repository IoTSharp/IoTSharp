...
| M5 | `🚧` | Release Center 第一版 | EdgeNode/Gateway 运行时软件发布、灰度批次、回滚、确认、回执和审计已具备平台第一版闭环；#056 设备脚本 OTA 平台侧已完成，#057 固件 OTA 继续推进设备侧 bootloader、重启确认和失败回滚验收。 |
| M6 | `🕒` | RuleChain 增强 | 标准节点、版本化、仿真、观测和审计增强，但不承担长周期发布；AI 节点与 MCP 扩写在此评估。 |
| T | `✅️` | SonnetDB 单依赖交付叙事（并行轨道） | README/文档首推单依赖体验路径；容量与可靠性基准报告；云边同底座参考部署。 |
...
M5 排在 Edge 和采集闭环之后。它不进入 RuleChain。#050 在 #046 最小模型上兼容扩展，不重建。第一版已闭环 EdgeNode/Gateway 运行时软件发布；#056 已完成平台侧 `DeviceScriptOta`、单 Device 目标、AssetScope/DeviceScope 范围展开、Gateway 投递通道和脚本 CRC 成功回执校验；当前重点转向 #057 `FirmwareOta` 设备侧 bootloader、分区写入、重启确认和失败回滚验收。
...
| #057 | `🚧` | 设备固件 OTA | 平台/设备 | 平台侧已追加 `FirmwareOta` EdgeTask、固件包元数据透传、设备范围发布和包/设备回执校验；设备侧 bootloader、分区写入、重启确认和失败回滚仍在 IoTEmbedded 仓验收。 |
...
| M5 | #050~#056 | 灰度/回滚执行端配合 | #057 固件 OTA；#056 设备侧执行器按 IoTEmbedded 仓库验收 | — |
...
M1（#010~#018）、M2（#020~#029）、M3（#030~#038）和 M4（#040~#048）已完成。M4 已具备采集配置版本快照、模板发布到 EdgeTask、执行端目标配置获取、执行回执、配置版本差异展示、失败重试、审计、平台侧最小软件包发布、IoTEdge 软件更新执行器和边缘数据断网缓存/续传。M5 Release Center 平台第一版（#050~#056）已形成 EdgeNode/Gateway 运行时软件发布和设备脚本 OTA 平台闭环；#057 正在推进设备固件 OTA 执行端合同与硬件回滚验收；并行轨道 T（#070~#072）已完成单依赖体验、基准报告和云边同底座参考部署。

M3 和 M4 已关闭，采集模板、边缘执行链路、最小软件包发布和边缘数据可靠性已经形成最小闭环。M5 在 #046 最小模型和 #047/#048 执行端能力上继续扩展发布计划、灰度、回滚、暂停继续、确认策略和审计，不进入 RuleChain。#056 设备脚本 OTA 的平台合同、范围发布、Gateway 投递通道和回执核对已落在 IoTSharp；当前重点是 #057 设备固件 OTA，固件真实执行、重启确认和硬件回滚在 IoTEmbedded 验收。

并行轨道 E（附属运行时硬化）与 A（离线 AI 底座）按带宽穿插推进，不阻塞主线；其中 #090（AI Provider 抽象）是 M6 #064/#065 与 #093 内网部署的前置。

#072 已在本仓库沉淀参考部署与平台侧验收文档；其他涉及 IoTEdge（#028、#038、#047、#048、#080~#084、#095）、IoTEmbedded（#056、#057、#085~#087）和 Tomur（#091、#092、#094）的事项在各自仓库实现，验收标准和契约版本以本路线图与 `IoTSharp.Contracts` 为准。
