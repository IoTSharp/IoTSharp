...
M1（#010~#018）、M2（#020~#029）、M3（#030~#038）和 M4（#040~#048）已完成。M4 已具备采集配置版本快照、模板发布到 EdgeTask、执行端目标配置获取、执行回执、配置版本差异展示、失败重试、审计、平台侧最小软件包发布、IoTEdge 软件更新执行器和边缘数据断网缓存/续传。M5 Release Center 平台第一版（#050~#056）已形成 EdgeNode/Gateway 运行时软件发布和设备脚本 OTA 平台闭环；#057 正在推进设备固件 OTA 执行端合同与硬件回滚验收；并行轨道 T（#070~#072）为文档层面工作，可随时穿插。
