...
| M5 | `🚧` | Release Center 第一版 | EdgeNode/Gateway 运行时软件发布、灰度批次、回滚、确认、回执和审计已具备平台第一版闭环；#056/#057 已进入设备脚本/固件 OTA 执行端合同与范围发布铺垫，设备侧执行器继续在 IoTEmbedded 落地。 |
...
M5 排在 Edge 和采集闭环之后。它不进入 RuleChain。#050 在 #046 最小模型上兼容扩展，不重建。第一版已闭环 EdgeNode/Gateway 运行时软件发布；当前转入 #056/#057，平台侧先补齐 `DeviceScriptOta`、`FirmwareOta`、单 Device 目标和 AssetScope/DeviceScope 范围展开，IoTEmbedded 执行器和 bootloader 配合在设备仓落地。
...
| #056 | `🚧` | 设备脚本 OTA | 平台/设备 | 平台侧已追加 `DeviceScriptOta` EdgeTask、`Device` 目标、脚本包元数据透传、AssetScope/DeviceScope 单设备展开和成功回执校验；IoTEmbedded 侧继续实现双槽切换、CRC/签名校验和启动失败回滚执行器。 |
...
M1（#010~#018）、M2（#020~#029）、M3（#030~#038）和 M4（#040~#048）已完成。M4 已具备采集配置版本快照、模板发布到 EdgeTask、执行端目标配置获取、执行回执、配置版本差异展示、失败重试、审计、平台侧最小软件包发布、IoTEdge 软件更新执行器和边缘数据断网缓存/续传。M5 Release Center 平台第一版（#050~#055）已形成 EdgeNode/Gateway 运行时软件发布闭环；#056/#057 正在推进设备脚本/固件 OTA 执行端合同与范围发布能力；并行轨道 T（#070~#072）为文档层面工作，可随时穿插。

M3 和 M4 已关闭，采集模板、边缘执行链路、最小软件包发布和边缘数据可靠性已经形成最小闭环。M5 在 #046 最小模型和 #047/#048 执行端能力上继续扩展发布计划、灰度、回滚、暂停继续、确认策略和审计，不进入 RuleChain。当前重点是 #056/#057 的设备脚本/固件 OTA：平台合同、范围发布、回执核对先落在 IoTSharp，脚本/固件真实执行和硬件回滚在 IoTEmbedded 验收。
